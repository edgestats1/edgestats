...
export function computeDominanceIndex(homeXG, awayXG, homeWin, awayWin, homeShots, awayShots) {
    const xgDiff = Math.abs(homeXG - awayXG);
    const shotDiff = Math.abs((homeShots ?? 0) - (awayShots ?? 0));
    const favWin = Math.max(homeWin, awayWin);
    return clamp(xgDiff * 0.32 + shotDiff * 0.035 + Math.max(0, favWin - 0.5) * 0.75, 0, 1);
}
function cleanSheetProb(oppLambda) {
    return round(poissonProbability(oppLambda, 0), 4) ?? Math.exp(-oppLambda);
}
function calibrateDrawProbability(raw, xgDiff, dominanceIndex) {
    const reasons = [];
    let draw = raw.draw;
...
export function runProbabilityEngine(input) {
    ...
    const dominanceIndex = computeDominanceIndex(homeLambda, awayLambda, rawPoisson.homeWin, rawPoisson.awayWin, input.homeShots, input.awayShots);
    const { probabilities, applied, reasons } = calibrateDrawProbability(rawPoisson, xgDiff, dominanceIndex);
    const mainReasons = [
