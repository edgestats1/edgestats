...
import { buildTeamMatchProfile, ratingGap, round, } from './teamRatings.js';
...
function collectLimitations(home, away) {
    const notes = [];
...
function formatThreatLine(threats) {
    if (!threats.length)
        return 'none identified from available data';
    return threats
        .map((t) => {
...
function buildBreakdown(home, away, expectedGoals, scoreline, rawPoissonLabel, probabilities, winner, v2Reasons) {
    const gap = ratingGap(home, away);
    const parts = [];
    parts.push(`${home.team} (${home.power.overallPowerScore ?? '—'} power) face ${away.team} (${away.power.overallPowerScore ?? '—'} power).`);
    if (gap != null) {
        parts.push(gap > 0
    ...
    parts.push(`Expected goals: ${expectedGoals.home ?? '—'}–${expectedGoals.away ?? '—'} from stat projection; V2 probability layer uses calibrated λ for win/draw/loss.`);
    parts.push(`Projected scoreline ${scoreline.selectedScoreline} (raw Poisson most likely: ${rawPoissonLabel}). ${scoreline.selectionReason}`);
    if (v2Reasons.length) {
        parts.push(`V2 calibration: ${v2Reasons.slice(0, 3).join('; ')}.`);
    }
    parts.push(`Outcome split: ${round(probabilities.homeWin * 100, 1)}% ${home.team} win, ${round(probabilities.draw * 100, 1)}% draw, ${round(probabilities.awayWin * 100, 1)}% ${away.team} win.`);
...
export function predictMatchFromProfiles(model, home, away, options = {}) {
    const pipeline = runPredictionPipeline({ model, home, away, options });
    const { statProjection, calibratedGoals, probabilityOutput, scoreline, scorelineDiagnostics, rawPoisson, } = pipeline;
    const expectedGoals = {
    ...
    const statConsistencyScore = finalizeConsistencyScore(statProjection, home, away, { home: scoreline.home, away: scoreline.away });
    ...
    return {
        ...
        breakdown: buildBreakdown(home, away, expectedGoals, scoreline, rawPoisson.label, probabilities, winner, v2Diagnostics.mainReasons),
        limitations,
        pairingNote: options.pairingNote ?? null,
    };
}
export function predictMatch(model, teamA, teamB, options = {}) {
...
export function buildKnockoutPairings(model) {
...
export function predictKnockoutRound(model) {
