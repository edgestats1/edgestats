...
function shotAccuracy(profile, model) {
...
function projectCornersForTeam(team, opponent, expectedShots, sanityChecks) {
    ...
    if (final >= 13 && !allow13Plus) {
        sanityChecks.push(`${team.team} corners capped from ${final} to 12 — 13+ requires avgCornersFor>=8, oppCornersAgainst>=7, expectedShots>=18`);
        final = 12;
    }
    ...
    if (final > 12 && allow13Plus) {
        sanityChecks.push(`${team.team} corners ${final} allowed — strong evidence: cornersFor=${teamBase}, oppCornersAgainst=${opponentAgainst}, expectedShots=${expectedShots}`);
    }
...
function projectShotsForTeam(team, opponent) {
...
function projectShotsOnTargetForTeam(team, expectedShots, accuracy, model) {
...
function profileShotAccuracyBase(profile, model) {
    return shotAccuracy(profile, model);
}
function projectGoalsForTeam(team, opponent, expectedSoT, model) {
...
function projectSavesForKeeper(keeperTeam, opponentSoT, opponentGoals) {
...
function computeConsistencyScore(home, away, projection, diagnostics) {
    let score = 100;
    const penalties = [];
...
export function projectMatchStats(model, home, away) {
    const sanityChecks = [];
...
export function finalizeConsistencyScore(projection, home, away, scorelineGoals) {
