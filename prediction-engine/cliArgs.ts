...
export interface ParsedPredictMatchArgs {
  teamA: string;
  teamB: string;
}

export function parsePredictMatchArgs(argv: string[]): ParsedPredictMatchArgs {
  ...
  if (positional.length === 1) {
    throw new Error(
      `Only one team provided ("${positional[0]}").\nUsage: npm run predict-match -- "<Team A>" "<Team B>"`,
    );
  }

  if (positional.length > 2) {
    throw new Error(
      `Expected exactly 2 team names, received ${positional.length}: ${positional.join(', ')}\n`
      + 'Usage: npm run predict-match -- "<Team A>" "<Team B>"',
    );
  }
...
export function isKnockoutCommand(argv: string[]): boolean {
