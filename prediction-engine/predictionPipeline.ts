...
import { projectMatchStats, finalizeConsistencyScore } from './statProjectionModel.js';
...
import { pickMostLikelyScoreline, determineWinner } from './scorelineModel.js';
...
function buildV2Diagnostics(
  probabilityOutput: ReturnType<typeof runProbabilityEngine>,
  scorelineDiagnostics: ReturnType<typeof selectRepresentativeScoreline>['diagnostics'],
...
export function runPredictionPipeline(ctx: PipelineContext): {
  statProjection: ReturnType<typeof projectMatchStats>;
  calibratedGoals: ReturnType<typeof calibrateExpectedGoals>;
  probabilityOutput: ReturnType<typeof runProbabilityEngine>;
  scoreline: ReturnType<typeof selectRepresentativeScoreline>['scoreline'];
  scorelineDiagnostics: ReturnType<typeof selectRepresentativeScoreline>['diagnostics'];
  v2Diagnostics: V2ModelDiagnostics;
  rawPoisson: ReturnType<typeof pickMostLikelyScoreline>;
} {
  ...
  const rawPoisson = pickMostLikelyScoreline(
    probabilityOutput.homeLambda,
    probabilityOutput.awayLambda,
  );
  ...
  const v2Diagnostics = buildV2Diagnostics(
    probabilityOutput,
