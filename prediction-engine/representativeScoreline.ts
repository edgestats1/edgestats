...
export interface RepresentativeScorelineInput {
...
function label(home: number, away: number): string {
  return `${home}-${away}`;
}

function uniqueCandidates(candidates: ScoreCandidate[]): ScoreCandidate[] {
...
export function goalBandFromXG(xG: number): number {
  if (xG < 0.45) return 0;
  if (xG < 1.25) return 1;
  if (xG < 2.05) return 2;
  if (xG < 2.85) return 3;
...
function adjustedGoalBand(
  xG: number,
  sot: number | null,
  finishingRating: number | null,
  oppGkRating: number | null,
  winProb: number,
): number {
...
function poissonProb(homeLambda: number, awayLambda: number, home: number, away: number): number {
  return poissonProbability(homeLambda, home) * poissonProbability(awayLambda, away);
}

function fitWinProbability(
...
function fitToXG(candidate: ScoreCandidate, homeXG: number, awayXG: number): number {
...
function fitToShotVolume(
  ...
  if (homeShots == null || awayShots == null) return 0.5;
  ...
  let score = 0.5;
  if (expectedSign === actualSign && expectedSign !== 0) score += 0.35;
  if (expectedSign === 0 && actualSign === 0) score += 0.25;

  const shotMag = Math.abs(shotDiff);
  if (shotMag >= 8 && Math.abs(goalDiff) >= 2 && actualSign === expectedSign) score += 0.2;
  if (shotMag >= 11) {
    if (shotDiff > 0 && candidate.home >= 2) score += 0.1;
    if (shotDiff < 0 && candidate.away >= 2) score += 0.1;
  }

  return clamp(score, 0, 1);
}

function fitToDefensiveProfile(
  candidate: ScoreCandidate,
  homeXG: number,
  awayXG: number,
  homeSoT: number | null,
  ...
  if (awayXG <= 0.5 && candidate.away === 0) score += 0.15;
  if (homeXG <= 0.5 && candidate.home === 0) score += 0.15;

  return clamp(score, 0, 1);
}

function fitToGoalkeeperProjection(
  candidate: ScoreCandidate,
  homeSaves: number | null,
  awaySaves: number | null,
  homeSoT: number | null,
...
function fitToCleanSheet(
  ...
  if (awayXG <= 0.7 && (awaySoT ?? 99) <= 2 && candidate.away === 0) score += 0.4;
  if (homeXG <= 0.7 && (homeSoT ?? 99) <= 2 && candidate.home === 0) score += 0.4;

  return clamp(score, 0, 1);
}

function fitToShotsOnTarget(
  candidate: ScoreCandidate,
  homeSoT: number | null,
  awaySoT: number | null,
): number {
  if (homeSoT == null || awaySoT == null) return 0.5;
...
function fitToAttackingDominance(
  candidate: ScoreCandidate,
  input: RepresentativeScorelineInput,
): number {
  ...
  if (dominance < 0.45) return 0.5;
  if (favMargin >= 2 && dominance >= 0.55) return 0.9;
  if (favMargin === 1 && dominance >= 0.65) return 0.35;
  if (favMargin >= 2) return 0.75;
  return 0.55;
}
...
function scoreCandidate(
  ...
  const defFit = fitToDefensiveProfile(
    candidate,
    input.homeXG,
    input.awayXG,
    input.homeSoT,
  ...
  const gkFit = fitToGoalkeeperProjection(
    candidate,
    input.homeSaves,
    input.awaySaves,
    input.homeSoT,
...
function buildDominanceCandidate(input: RepresentativeScorelineInput): ScoreCandidate | null {
  ...
  if (favWin < 0.65) return null;
  if (favXG - underXG < 0.9) return null;
  if ((favShots ?? 0) - (underShots ?? 0) <= 6) return null;
  if (underXG > 0.7) return null;
  if ((underSoT ?? 99) > 2) return null;
  ...
  let favGoals = adjustedGoalBand(
    favXG,
    favIsHome ? input.homeSoT : input.awaySoT,
    favProfile.power.finishingRating,
    oppGk,
    favWin,
  );
...
function buildCleanSheetCandidate(input: RepresentativeScorelineInput): ScoreCandidate | null {
  const favIsHome = input.probabilities.homeWin >= input.probabilities.awayWin;
  const favWin = favIsHome ? input.probabilities.homeWin : input.probabilities.awayWin;
  const favXG = favIsHome ? input.homeXG : input.awayXG;
  const underXG = favIsHome ? input.awayXG : input.homeXG;
  ...
  if (!cleanSheetLikely) return null;
...
function buildShotVolumeCandidate(input: RepresentativeScorelineInput): ScoreCandidate | null {
  const favIsHome = input.probabilities.homeWin >= input.probabilities.awayWin;
  const favShots = favIsHome ? input.homeShots : input.awayShots;
  const favXG = favIsHome ? input.homeXG : input.awayXG;
  const underXG = favIsHome ? input.awayXG : input.homeXG;

  if ((favShots ?? 0) < 15 || favXG < 1.4) return null;
...
function buildUnderdogScorerCandidate(input: RepresentativeScorelineInput): ScoreCandidate | null {
  const favIsHome = input.probabilities.homeWin >= input.probabilities.awayWin;
  const favWin = favIsHome ? input.probabilities.homeWin : input.probabilities.awayWin;
  if (favWin < 0.55) return null;
  ...
  if (!strongUnderdog) return null;

  const favXG = favIsHome ? input.homeXG : input.awayXG;
  const underXG = favIsHome ? input.awayXG : input.homeXG;
  if (favXG - underXG < 0.5) return null;

  let favGoals = Math.max(2, goalBandFromXG(favXG));
  if (favDef >= 58 && underAttack >= 60) favGoals = Math.min(favGoals, 2);

  const underGoals = underXG >= 0.4 || underAttack >= 58 ? 1 : 0;
  const home = favIsHome ? favGoals : underGoals;
  const away = favIsHome ? underGoals : favGoals;

  if (underGoals === 0) return null;
  return { home, away, label: label(home, away), source: 'underdog-scorer-adjusted' };
}

function buildMultiGoalDominanceCandidates(input: RepresentativeScorelineInput): ScoreCandidate[] {
  ...
  if (favWin < 0.65 || xgDiff < 0.9 || dom < 0.5) return [];
...
function buildDrawCandidate(input: RepresentativeScorelineInput): ScoreCandidate | null {
  ...
  if (winDiff > 0.10 || xgDiff > 0.4) return null;
...
function buildRoundedXGCandidate(input: RepresentativeScorelineInput): ScoreCandidate {
...
function buildSelectionReason(
  selected: ScorelineCandidateScore,
  input: RepresentativeScorelineInput,
  flags: {
...
export function selectRepresentativeScoreline(
  input: RepresentativeScorelineInput,
): { scoreline: ScorelineOutput; diagnostics: ScorelineDiagnostics } {
  ...
  const goalBandHome = adjustedGoalBand(
    input.homeXG,
    input.homeSoT,
    input.home.power.finishingRating,
    input.away.power.goalkeeperRating,
    input.probabilities.homeWin,
  );
  const goalBandAway = adjustedGoalBand(
    input.awayXG,
    input.awaySoT,
    input.away.power.finishingRating,
    input.home.power.goalkeeperRating,
  ...
  const goalBandHomeCandidate: ScoreCandidate = {
    home: goalBandHome,
    away: goalBandAway,
  ...
  if (underdogGoalRuleApplied && underdogCandidate) {
    ...
    scored = scored.map((s) => {
      if (s.label === uLabel) return { ...s, totalScore: (s.totalScore ?? 0) + 0.10 };
  ...
  const best = scored[0] ?? scoreCandidate(
    { home: rawPoisson.home, away: rawPoisson.away, label: rawPoisson.label, source: 'poisson-fallback' },
    input,
    topPoissonProb,
  );
  ...
  const selectionReason = buildSelectionReason(
    best,
    input,
    { dominanceRuleApplied, cleanSheetRuleApplied, drawRuleApplied, upsetRuleApplied, underdogGoalRuleApplied },
    input.home.team,
    input.away.team,
  );

  const scoreline: ScorelineOutput = {
  ...
  const diagnostics: ScorelineDiagnostics = {
