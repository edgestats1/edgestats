...
function blendNullable(weights: Array<[number | null, number]>): number | null {
  let total = 0;
  let weightSum = 0;
...
export function getTopPoissonScorelines(
  homeLambda: number,
  awayLambda: number,
  limit = 10,
): Array<{ home: number; away: number; label: string; probability: number }> {
  const scores: Array<{ home: number; away: number; label: string; probability: number }> = [];
...
export function determineWinner(
  ...
  if (scoreline.home > scoreline.away) return homeTeam;
  if (scoreline.away > scoreline.home) return awayTeam;
  if (probabilities.homeWin >= probabilities.awayWin) return homeTeam;
  if (probabilities.awayWin > probabilities.homeWin) return awayTeam;
  return 'Draw';
}
...
export function projectCards(
  ...
  home: TeamMatchProfile,
  away: TeamMatchProfile,
): { total: number | null; home: number | null; away: number | null } {
...
export function projectExpectedShots(
  home: TeamMatchProfile,
  away: TeamMatchProfile,
): Record<string, { total: number | null; onTarget: number | null }> {
...
export function projectGoalkeeperSaves(
  home: TeamMatchProfile,
  away: TeamMatchProfile,
  ...
  const homeSaves = estimateSaves(
    homeGk,
    awayShotsOnTarget,
    expectedGoals.away,
    away.finishingEfficiency,
  );
  const awaySaves = estimateSaves(
    awayGk,
    homeShotsOnTarget,
    expectedGoals.home,
    home.finishingEfficiency,
  );
...
function estimateSaves(
  keeper: TeamMatchProfile['primaryGoalkeeper'],
  opponentShotsOnTarget: number | null,
  opponentExpectedGoals: number | null,
  opponentFinishingEfficiency: number | null,
): number | null {
